...
class Gitea(GiteaBase):
    ...
    def create_user(self, username, email, password, must_change_password=False, token=None, redirect_url=None):
        ...
        if redirect_url:
            user.create_oauth(username, password, redirect_url)
        return user
...
class User(GiteaBase):
    ...
    def create_token(self, token, name='token'):
        ...
        con.commit()
        con.close()
    def create_oauth(self, username, password, redirect_url):
        data = {
            "name": f'{username}-oauth',
            "redirect_uris": [f"{redirect_url}"]
        }
        res = requests.post('http://localhost:3000/api/v1/user/applications/oauth2', headers=self.create_header(username, password), json=data)
        return res
...
class Org(GiteaBase):
    ...
    def create_team(self, name, permission, members):
        ...
        for username in members:
            print(username)
            res = self.put(f"/teams/2/members/{username}")
...
class Repo(GiteaBase):
    ...
    def push_code(self, git_repo_path):
        try:
            ...
            repo.git.push(f'https://gitadmin:escrow@git.devops.hkn/{self.org}/{self.name}.git', '--tags', '-u', self.default_branch)
